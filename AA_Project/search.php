<?php	include "inc/header.php";
		...
		$sql_search = "SELECT * FROM files 
					   WHERE thumb LIKE '%$search%' 
					   OR filename LIKE '%$search%' 
					   OR size LIKE '%$search%' 
					   OR uploaded LIKE '%$search%'";
...
<?php 	
			if (mysqli_num_rows($result) > 0) {
				while ($file = mysqli_fetch_array($searchResult)) {
...
?>
				...
				<tfoot>
					<tr>
						<td></td>
						<td><?php echo mysqli_num_rows($searchResult) ?> Results</td>
						<td></td>
						<td></td>
					</tr>
				</tfoot>
